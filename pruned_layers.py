...
class PruneLinear(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        perc = np.percentile(np.abs(weights),q)
        bools = np.abs(weights) > perc
        self.mask = bools.astype(int)
        self.sparsity = 1 - np.sum(self.mask)/self.mask.size
        weights = np.multiply(weights, self.mask)
        self.linear.weight.data = torch.from_numpy(weights).float().to(device)
        pass
    ...
    def prune_by_std(self, s=0.25):
        ...
        """
        weights = self.linear.weight.data.cpu().numpy()
        thresh = s*np.std(weights)
        bools = np.abs(weights) > thresh
        self.mask = bools.astype(int)
        self.sparsity = 1 - np.sum(self.mask)/self.mask.size
        weights = np.multiply(weights, self.mask)
        self.linear.weight.data = torch.from_numpy(weights).float().to(device)
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        self.sparsity = 1 - np.sum(self.mask)/self.mask.size
        weights = np.multiply(weights, self.mask)
        self.conv.weight.data = torch.from_numpy(weights).float().to(device)
        pass
