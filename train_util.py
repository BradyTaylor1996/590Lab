...
def finetune_after_prune(net, epochs=100, batch_size=128, lr=0.01, reg=5e-4):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for n, m in net.named_modules():
                if isinstance(m, PrunedConv):
                    first_grads = m.conv.weight.grad.cpu().numpy()
                    new_grads = np.multiply(first_grads, m.mask)
                    m.conv.weight.grad = torch.from_numpy(new_grads).float().to(device)
                elif isinstance(m, PruneLinear):
                    first_grads = m.linear.weight.grad.cpu().numpy()
                    new_grads = np.multiply(first_grads, m.mask)
                    m.linear.weight.grad = torch.from_numpy(new_grads).float().to(device)
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            print("Saving...")
            torch.save(net.state_dict(), "net_after_pruning1.pt")
